          ...
          {load ? (
            ...
            error ? (
              ...
              userData.map((contact) => (
                <div onClick={() => handleClick(contact)} key={contact._id} style={{ marginLeft: '45px', display: "flex", alignItems: 'center', gap: "5px" }}>
                  <UserCard
                    username={contact?.username}
                    avatar={contact?.avatar}
                    message={contact?.bio}
                  /> 
                  <IoAddCircleOutline size={35} style={{ color: 'green', cursor: 'pointer' }}
/>
                </div>
              ))
            )
